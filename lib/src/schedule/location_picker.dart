...
class _LocationPickerState extends State<LocationPicker> {
  ...
  ].map((json) => District.fromJson(json)).toList();
    ...
    requestPermission().then((granted) {
      if (granted) {
        getCurrentLocation().then((locationData) {
          ...
          getCurrentLocationDetails().then((Map<String, dynamic> details) {
            if (details['district'] != null) {
              String districtName = details['district'].split(' ').first;
              List<District> matchedDistricts = districtList.where((district) =>
              district.name == districtName || district.bn_name == districtName).toList();
              if (matchedDistricts.isNotEmpty) {
                for (District district in matchedDistricts) {
                  print("Matched District:");
                  print("ID: ${district.id}");
                  print("Name: ${district.name}");
                  print("Bengali Name: ${district.bn_name}");
                  print("Latitude: ${district.lat}");
                  print("Longitude: ${district.lon}");
                  print("URL: ${district.url}");
                  print("-----------------------------");
                }
              } else {
                print("No district found in the list matching '$districtName'");
              }
            } else {
  ...
  setLocation() async {
    ...
    if (matchedDistrict!.id.isNotEmpty) {
      selectedLocation = matchedDistrict;
