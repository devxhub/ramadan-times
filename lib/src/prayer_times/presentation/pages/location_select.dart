              ...
              children: [
                ...
                DropdownButton<Country>(
                  ...
                  onChanged: (Country? newValue) {
                    context.read<PrayerTimeBloc>().add(PrayerTimeEvent.selectCountry(context: context, country:newValue??Country()));
                  },
                ),
                      ...
                      return state.when(
                        ...
                        data: (data) => DropdownButton<District>(
                          ...
                          onChanged: (District? value) async {
                           context.read<PrayerTimeBloc>().add(PrayerTimeEvent.selectCity(context: context, district: value??District()));
                          },
                        ),
                        error: (e) => Container(),
                      );
                  ...
                  onTap: (){
                    if(prayerTimeState.selectedCountry?.name.toString().toLowerCase()=="bangladesh"){
                      if(prayerTimeState.selectedDistrict?.name?.isNotEmpty==true){
                        context.read<PrayerTimeBloc>().add(PrayerTimeEvent.submitLocation(context: context, userCoordinator:UserCoordinator(
...
context.read<PrayerTimeBloc>().add(PrayerTimeEvent.isDistrictSelected(context: context, isDistrictSelected: false));
                      }
                  ...
                  child: Container(
                    ...
                    width: 300.w,
                    color: Colors.red,
                    child: Center(child: prayerTimeState.prayerTimeStatus==PrayerTimeStatus.initial?CircularProgressIndicator(): Text("Save")),
                  ),
                )
              ],
            ),
